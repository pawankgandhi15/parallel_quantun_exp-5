...
\subsection{Simulation Infrastructure \& Hybrid Autodiff Framework}
All hybrid quantum-classical computations, variational state-vector evaluations, and parameter-shift gradient derivations are implemented using the PennyLane quantum software ecosystem \cite{bergholm2018pennylane} integrated with PyTorch. Quantum circuits are executed via PennyLane's high-performance C++ backend (`lightning.qubit`).
...
\begin{enumerate}
    \item \textbf{Expressibility ($\mathrm{Expr} \downarrow$):} Introduced by Sim, Johnson, and Aspuru-Guzik \cite{sim2019expressibility}, this metric quantifies the divergence between sampled fidelity distribution $P_{\mathrm{PQC}}(F)$ and the Haar measure $P_{\mathrm{Haar}}(F) = 15(1-F)^{14}$ (for $d=16$):
    \begin{equation}
...
To execute this workload efficiently in simulation, our codebase leverages OpenMP multi-threading within PennyLane's C++ state-vector backend (`lightning.qubit`) \cite{bergholm2018pennylane} combined with batch parallel dispatch across high-performance compute nodes. On a dual AMD EPYC 7763 64-core system, forward batch evaluation completes in $48\,\mathrm{ms}$, rendering extensive multi-dataset benchmarking feasible.
