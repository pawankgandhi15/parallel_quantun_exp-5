...
\begin{align}
RX(\theta) &= \exp(-i \theta X / 2) = \cos(\theta/2) \mathbb{I} - i \sin(\theta/2) X, \label{eq:rx_gate} \\
RY(\theta) &= \exp(-i \theta Y / 2) = \cos(\theta/2) \mathbb{I} - i \sin(\theta/2) Y, \label{eq:ry_gate} \\
RZ(\theta) &= \exp(-i \theta Z / 2) = \begin{bmatrix} e^{-i\theta/2} & 0 \\ 0 & e^{i\theta/2} \end{bmatrix}. \label{eq:rz_gate}
\end{align}
...
&= \sum_{m=0}^{\infty} \frac{(-1)^m (\frac{r\theta}{2})^{2m}}{(2m)!} \mathbb{I} \nonumber \\
&\quad - i \sum_{m=0}^{\infty} \frac{(-1)^m (\frac{r\theta}{2})^{2m+1}}{(2m+1)!} \frac{G}{r} \nonumber \\
...
&= \prod_{k=0}^3 \left| \cos(\pi p_k / 2 + \pi/4)\cos(\pi p_k' / 2 + \pi/4) \right. \nonumber \\
&\quad \left. + \sin(\pi p_k / 2 + \pi/4)\sin(\pi p_k' / 2 + \pi/4) \right|^2 \nonumber \\
...
\end{equation}
\begin{align}
U_{\mathrm{ent}}^{(1)}(\mathbf{w}_{\mathrm{ent}}^{(1)}) &= CRX_{3 \to 0}(w_{\mathrm{ent}}^{(1)}[3]) \cdot CRX_{2 \to 3}(w_{\mathrm{ent}}^{(1)}[2]) \nonumber \\
&\quad \cdot CRX_{1 \to 2}(w_{\mathrm{ent}}^{(1)}[1]) \cdot CRX_{0 \to 1}(w_{\mathrm{ent}}^{(1)}[0]).
\label{eq:ent_layer1}
\end{align}
...
\begin{align}
U_{\mathrm{ent}}^{(2)}(\mathbf{w}_{\mathrm{ent}}^{(2)}) &= CRX_{0 \to 1}(w_{\mathrm{ent}}^{(2)}[3]) \cdot CRX_{3 \to 0}(w_{\mathrm{ent}}^{(2)}[2]) \nonumber \\
&\quad \cdot CRX_{2 \to 3}(w_{\mathrm{ent}}^{(2)}[1]) \cdot CRX_{1 \to 2}(w_{\mathrm{ent}}^{(2)}[0]).
\label{eq:ent_layer2}
\end{align}
...
\begin{align}
\mathbf{h}_1 &= \mathrm{Dropout}_{0.2}\left( \mathrm{ReLU}(\mathbf{W}_1 \mathbf{y}_{\mathrm{flat}} + \mathbf{b}_1) \right), \label{eq:fc1} \\
\mathbf{h}_2 &= \mathrm{Dropout}_{0.2}\left( \mathrm{ReLU}(\mathbf{W}_2 \mathbf{h}_1 + \mathbf{b}_2) \right), \label{eq:fc2} \\
...
\mathrm{Var}_{\boldsymbol{\theta}}\left[ \frac{\partial \langle M \rangle}{\partial \theta_k} \right] &= \frac{\mathrm{Tr}\left([H_k, \rho_0]^2\right)}{d^2 - 1} \nonumber \\
...
\begin{align}
\mathrm{Var}_{\boldsymbol{\theta}}\left[ \frac{\partial \langle Z_k \rangle}{\partial \theta_k} \right] &= \frac{2^n}{2^{2n} - 1} \mathrm{Tr}([H_k, \rho_0]^2) \nonumber \\
...
\begin{enumerate}
    ...
    \end{equation}
    where $\rho_k = \mathrm{Tr}_{\setminus k} |\psi\rangle\langle\psi|$.
...
\begin{equation}
\lim_{p \to 1} z_k(i, j) = \mathrm{Tr}\left( Z_k \frac{\mathbb{I}_{16}}{16} \right) = 0, \quad \forall k.
\end{equation}
...
\begin{align}
\mathcal{N}_{\mathrm{eval}} &= B \times \left(\frac{H}{s}\right) \times \left(\frac{W}{s}\right) \nonumber \\
&= 32 \times 14 \times 14 = 6,272 \text{ QNodes/batch}.
...
\begin{equation}
T_{\mathrm{circuit}} \approx (12 \times 200\,\mathrm{ns}) + (4 \times 30\,\mathrm{ns}) = 2.52\,\mu\mathrm{s}.
