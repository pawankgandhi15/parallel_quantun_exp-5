...
\begin{IEEEkeywords}
Hybrid Quantum-Classical Computing, Parameterized Quantum Circuits (PQCs), Quantum Convolutional Neural Networks (QCNN), Barren Plateaus, Noise Resilience, Image Classification, Parameter-Shift Rule, NISQ Algorithms.
\end{IEEEkeywords}
...
\begin{enumerate}
    ...
    \item \textbf{Theoretical & Empirical Gradient Variance Preservation:} We provide a theoretical analysis demonstrating that the shallow parallel topology avoids the 2-design trap, ensuring bounded gradient variances and positive definite Fisher information matrices throughout optimization.
...
\textbf{Theorem 1 (Exact Parameter-Shift Evaluation):} Let $\mathcal{U}(\theta) = \exp(-i \frac{\theta}{2} G)$ denote a parameterized unitary gate whose generator $G$ possesses two distinct eigenvalues $\pm r$. For an observable expectation value $f(\theta) = \langle \psi_0 | \mathcal{U}^\dagger(\theta) M \mathcal{U}(\theta) | \psi_0 \rangle$, the analytic derivative is given exactly by:
\begin{equation}
\frac{d f(\theta)}{d\theta} = r \left[ f\left(\theta + \frac{\pi}{4r}\right) - f\left(\theta - \frac{\pi}{4r}\right) \right].
\label{eq:param_shift_theorem}
\end{equation}

\textit{Proof Sketch:} Because $G^2 = r^2 \mathbb{I}$, the matrix exponential decomposes via Euler's identity into:
\begin{equation}
\mathcal{U}(\theta) = \cos\left(\frac{r\theta}{2}\right) \mathbb{I} - i \sin\left(\frac{r\theta}{2}\right) \frac{G}{r}.
\end{equation}
Expanding $f(\theta)$ into trigonometric Fourier series yields $f(\theta) = A + B \cos(r\theta) + C \sin(r\theta)$. Differentiating directly gives $f'(\theta) = -r B \sin(r\theta) + r C \cos(r\theta)$. Applying the trigonometric angle-sum identity with shift $s = \frac{\pi}{4r}$ proves the theorem.

For canonical Pauli generators ($G \in \{X, Y, Z\}$), the eigenvalues are $\pm 1$, corresponding to $r = 1/2$. Thus, the macroscopic shift is $s = \frac{\pi}{4(1/2)} = \frac{\pi}{2}$, yielding the exact formula:
\begin{equation}
...
\label{eq:canonical_param_shift}
\end{equation}

\subsection{Kraus Operator Formalism for Open Quantum Systems}
On physical NISQ processors, interaction with an uncontrolled thermal environment converts pure states into mixed states, represented by density operators $\rho \in \mathcal{S}(\mathcal{H})$ satisfying $\rho \ge 0$ and $\mathrm{Tr}(\rho) = 1$. The state evolution is modeled by a Completely Positive Trace-Preserving (CPTP) dynamical map $\mathcal{E}$ via Kraus decomposition:
\begin{equation}
\mathcal{E}(\rho) = \sum_{k} E_k \rho E_k^\dagger, \quad \text{subject to} \quad \sum_{k} E_k^\dagger E_k = \mathbb{I}.
\label{eq:kraus_representation}
\end{equation}
The physical noise models evaluated in this work are characterized by the following Kraus sets:
\begin{enumerate}
    \item \textbf{Bit-Flip Channel (Pauli-X Error):}
    \begin{equation}
    E_0 = \sqrt{1 - p} \, \mathbb{I}, \quad E_1 = \sqrt{p} \, X.
    \label{eq:kraus_bitflip}
    \end{equation}
    \item \textbf{Phase-Flip Channel (Pauli-Z Dephasing):}
    \begin{equation}
    E_0 = \sqrt{1 - p} \, \mathbb{I}, \quad E_1 = \sqrt{p} \, Z.
    \label{eq:kraus_phaseflip}
    \end{equation}
    \item \textbf{Depolarizing Channel (Isotropic Decoherence):}
    \begin{equation}
...
\subsection{Theoretical Analysis: Barren Plateau Prevention via Width Partitioning}
To explain why QC-CNN-Parallel fundamentally avoids barren plateaus, we establish the following proposition regarding gradient variance behavior in parallel versus sequential hybrids.

\textbf{Proposition 1 (Gradient Variance Preservation):} Let $\mathcal{U} = \prod_{l=1}^L U_l(\boldsymbol{\theta}_l)$ denote an ansatz on $n$ qubits. If $\mathcal{U}$ forms a unitary 2-design with respect to the Haar measure, then for any local observable $M$:
\begin{equation}
\mathrm{Var}_{\boldsymbol{\theta}}\left[ \frac{\partial \langle M \rangle}{\partial \theta_k} \right] \le \frac{2}{2^{2n} - 1} \mathrm{Tr}(M^2) \in \mathcal{O}\left(\frac{1}{2^{2n}}\right).
\label{eq:haar_variance_bound}
\end{equation}
In QC-CNN-Parallel, the variational circuit depth is strictly bounded at $L=2$. Because forming an approximate 2-design on $n=4$ qubits requires depth $L^* \ge \Omega(n^2) = 16$ \cite{haferkamp2022linear}, Circuit~11 does not satisfy the Haar 2-design condition. Consequently, the gradient variance is bounded away from zero:
\begin{equation}
\mathrm{Var}_{\mathbf{w}}\left[ \frac{\partial \langle Z_k \rangle}{\partial w_m} \right] \ge \delta > 0,
\label{eq:non_vanishing_gradient}
\end{equation}
guaranteeing healthy trainability. Furthermore, the total hybrid gradient vector $\mathbf{g}_{\mathrm{total}} = [\nabla_{\mathbf{W}_c} \mathcal{L}, \nabla_{\mathbf{w}} \mathcal{L}, \nabla_{\mathrm{dense}} \mathcal{L}]^T$ satisfies:
\begin{equation}
...
\label{eq:gradient_norm_lower_bound}
\end{equation}
Thus, even under pathological environmental noise where quantum features suffer degradation, the classical pathway guarantees uninterrupted non-zero gradient descent.
...
The comprehensive training routine is formalized in Algorithm~\ref{alg:training_loop}.
...
\subsection{Baseline Architectures for Comparative Evaluation}
To place QC-CNN-Parallel in context with established hybrid and classical visual models, we implement and benchmark against the following architectures:
\begin{enumerate}
    \item \textbf{Classical CNN (LeNet-5 Adapted):} A two-stage convolutional network comprising Conv1 ($1 \to 4$, $2\times 2$, stride 2, 20 params) and Conv2 ($4 \to 12$, $3\times 3$, stride 1, padding 1, 444 params), totaling 464 convolutional parameters.
    \item \textbf{HQNN-Quanv (Senokosov \textit{et al.} \cite{senokosov2024quantum}):} A sequential hybrid model passing images through an initial 4-qubit quanvolutional filter followed by a 448-parameter classical convolutional block.
    \item \textbf{QC-CNN (Henderson \textit{et al.} \cite{henderson2020quanvolutional}):} The canonical quanvolutional architecture where localized $2\times 2$ pixel patches are transformed by random or trained PQCs.
    \item \textbf{VCNN (Huang \textit{et al.} \cite{huang2021experimental}):} A variational convolutional model using multi-layer variational quantum gates with alternating $CZ$ entanglement.
    \item \textbf{QC-ResNet (Shi \textit{et al.} \cite{shi2022quantum}):} A hybrid residual network featuring quantum skip connections around classical convolutional blocks.
    \item \textbf{QC-Inception (Wang \textit{et al.} \cite{wang2022quantum}):} A multi-scale quantum inception module combining parallel $2\times 2$ and $3\times 3$ quantum receptive fields.
\end{enumerate}
...
To understand where the parallel quantum feature maps contribute maximum discriminative value, Table~\ref{tab:per_class_breakdown} reports the per-class precision, recall, and F1-scores on Fashion-MNIST comparing QC-CNN-Parallel against the 464-parameter classical LeNet-5 baseline.
...
The most significant performance improvements occur on visually ambiguous apparel classes: Class~6 (Shirt) experiences a $+7.6\%$ F1 gain ($0.522 \to 0.598$), and Class~2 (Pullover) experiences a $+6.1\%$ F1 gain ($0.649 \to 0.710$). In classical architectures, shirts and T-shirts exhibit near-identical bounding outlines, frequently confusing spatial filters. The quantum branch, by calculating phase rotations over $2 \times 2$ pixel neighborhoods, resolves subtle weave-level texture gradients across the Hilbert state space, providing orthogonal feature vectors that disambiguate fine clothing patterns.
...
For physical deployment on superconducting quantum hardware (e.g., IBM Quantum Eagle or Heron processors), Circuit~11 must be decomposed into native basis gates $\{\sqrt{X}, X, RZ, \mathrm{CNOT}\}$.
